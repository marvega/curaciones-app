...
TypeORM `synchronize: true` handles both changes automatically (new column + new table). Since this spec only ADDS schema (no drops/renames), it is safe with synchronize. No phased deploy needed.

```sql
-- Production DDL (if running manually)
BEGIN;

ALTER TABLE patients ADD COLUMN status VARCHAR DEFAULT 'active' NOT NULL;

CREATE TABLE patient_status_changes (
...
All existing patients receive `status = 'active'` via the column default.
