...
export interface ListWoundNotesInput {
  patientId: number;
  cursor?: string;
  limit?: number;
}

export async function listWoundNotesHandler(input: ListWoundNotesInput, ctx: ToolContext): Promise<ToolResult> {
  const r = await ctx.backend.request({
    method: 'GET',
    path: `/api/wound-notes/patient/${input.patientId}`,
    bearer: ctx.bearer,
